...
class Reminders_Buttons(View):
    ...
    async def yes_button(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            from utils.db.auction_reminder_db import (
                fetch_auction_reminder,
                upsert_auction_reminder,
            )

            reminder = await fetch_auction_reminder(
                self.bot, self.ends_on, self.member.id
            )
            if not reminder:
                await upsert_auction_reminder(
                    self.bot, self.ends_on, self.member.id, self.member.name, True
                )
            else:
                await update_auction_reminder_alarm(
                    self.bot, self.ends_on, self.member.id, True
                )
            content = "✅ Auction reminder has been set!"
            if not interaction.response.is_done():
                await interaction.response.edit_message(
                    content=content, view=None, embed=None
                )
            else:
            ...
            pretty_log(
                tag="error",
                message=f"Failed to update auction reminder for user ID {self.member.id} and ends_on {self.ends_on}: {e}",
            )
            await interaction.response.send_message(
                "❌ Failed to update auction reminder. Please try again later.",
                ephemeral=True,
            )
    ...
    async def no_button(self, interaction: discord.Interaction, button: Button):
        ...
        content = (
            f"{self.member.mention}, ⚠️ Don't forget to set an auction reminder later!"
        )
        if not interaction.response.is_done():
            await interaction.response.edit_message(
                content=content, view=None, embed=None
...
async def send_reminder_prompt_embed(
    ...
    embed = discord.Embed(
        ...
        description=(
            f"Have you set an alarm for the auction that will end on <t:{ends_on}:f>?"
        ),
        # Pastel pink
        color=0xFFB6C1,
    )
    view = Reminders_Buttons(bot, member, ends_on)
...
async def process_auction_timestamps(
    ...
    for ends_on in timestamp_list:
        ...
        if key not in auction_reminder_cache:
            ...
            if not is_timestamp_more_than_5_hours_away_cache(ends_on, member.id):
