...
from config.settings import CHECKLIST_SETTINGS_MAP, POKEMEOW_APPLICATION_ID
...
# 🌸──────────────────────────────────────────────
async def missing_pokemon_box_func(
    bot: commands.Bot,
    ...
    # Check if user is in Checklist Settings Map
    if user_id not in CHECKLIST_SETTINGS_MAP:
        await handler.error(
            "You do not have checklist settings configured. Please contact an admin."
        )
        return
